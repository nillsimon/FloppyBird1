...
public class Maze implements Drawable {
    ...
    private void generateMaze(){
        ...
        while (stack.size() > 0) {
            ...
            //Left
            if(current.x > 2){
                if(!isUsesCell(current.x -2, current.y)) {
                ...
                //Top
            if(current.y > 2){
                if(!isUsesCell(current.x, current.y - 2)){
        ...
        public boolean isCrosroad(int x, int y){
            return isUsesCell(x, y);
        }

        public boolean canPlayerGoTo(int y, int x){
        return array[y][x];
        }

    private boolean isUsesCell(int x, int y){
